...
export default function whatstatus() {
  ...
  const getByStatus = async () => {
    ...
    try {
      ...
      const response = await fetch(`${NGROK_URL}/get_movies_series_by_status?id_token=${idToken}`, {
        method: 'GET',
        headers: {
  ...
  return (
    <CustomKeyboardView>
      ...
      <View className='mt-4 mx-4 pt-4 pb-6 px-5 gap-3 bg-slate-700 rounded-2xl'>
        ...
        <View>
          {watching.length === 0 ? (
            <Text className="text-white">No movies/series to watch right now.</Text>
          ) : (
        ...
        <View>
          {planning.length === 0 ? (
            <Text className="text-white">No movies/series planned to watch yet.</Text>
          ) : (
